"use client";

import { motion } from "framer-motion";

const quickLinks = [
  { label: "About Us", href: "#about" },
  { label: "Our Impact", href: "#impact" },
  { label: "Our Story", href: "#story" },
  { label: "Gallery", href: "#gallery" },
  { label: "Become a Member", href: "mailto:info@ijebuigbodiaspora.org" },
];

const socialLinks = [
  {
    label: "Facebook",
    href: "https://facebook.com",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    label: "Twitter / X",
    href: "https://twitter.com",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    label: "Instagram",
    href: "https://instagram.com",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
  {
    label: "YouTube",
    href: "https://youtube.com",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer style={{ background: "#111111" }}>
      {/* Gold top border */}
      <div className="w-full h-1" style={{ background: "#C9A227" }} />

      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand */}
          <div className="lg:col-span-2">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
            >
              <div className="flex items-center gap-3 mb-4">
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold"
                  style={{ background: "#C9A227", color: "#0B3D91" }}
                >
                  II
                </div>
                <div>
                  <div
                    className="font-bold text-lg leading-tight"
                    style={{ color: "#ffffff" }}
                  >
                    Ijebu Igbo Descendants
                  </div>
                  <div className="text-sm" style={{ color: "#C9A227" }}>
                    in Diaspora
                  </div>
                </div>
              </div>

              <p className="text-sm leading-relaxed mb-6" style={{ color: "#aaaaaa" }}>
                A global community organisation dedicated to uniting Ijebu Igbo
                descendants worldwide, preserving our heritage, and driving
                sustainable development for Ijebu Igbo town, Ogun State,
                Nigeria.
              </p>

              {/* Social icons */}
              <div className="flex gap-3">
                {socialLinks.map((link, i) => (
                  <a
                    key={i}
                    href={link.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={link.label}
                    className="w-9 h-9 rounded-full flex items-center justify-center transition-colors duration-300"
                    style={{ background: "#222222", color: "#aaaaaa" }}
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLAnchorElement).style.background = "#C9A227";
                      (e.currentTarget as HTMLAnchorElement).style.color = "#111111";
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLAnchorElement).style.background = "#222222";
                      (e.currentTarget as HTMLAnchorElement).style.color = "#aaaaaa";
                    }}
                  >
                    {link.icon}
                  </a>
                ))}
              </div>
            </motion.div>
          </div>

          {/* Quick Links */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.1 }}
          >
            <h4
              className="text-sm font-bold tracking-widest uppercase mb-6"
              style={{ color: "#C9A227" }}
            >
              Quick Links
            </h4>
            <ul className="flex flex-col gap-3">
              {quickLinks.map((link, i) => (
                <li key={i}>
                  <a
                    href={link.href}
                    className="text-sm transition-colors duration-200"
                    style={{ color: "#aaaaaa" }}
                    onMouseEnter={(e) =>
                      ((e.target as HTMLAnchorElement).style.color = "#C9A227")
                    }
                    onMouseLeave={(e) =>
                      ((e.target as HTMLAnchorElement).style.color = "#aaaaaa")
                    }
                  >
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </motion.div>

          {/* Contact */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <h4
              className="text-sm font-bold tracking-widest uppercase mb-6"
              style={{ color: "#C9A227" }}
            >
              Contact Us
            </h4>
            <ul className="flex flex-col gap-4">
              <li>
                <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "#555555" }}>
                  General Enquiries
                </div>
                <a
                  href="mailto:info@ijebuigbodiaspora.org"
                  className="text-sm transition-colors duration-200"
                  style={{ color: "#aaaaaa" }}
                  onMouseEnter={(e) =>
                    ((e.target as HTMLAnchorElement).style.color = "#C9A227")
                  }
                  onMouseLeave={(e) =>
                    ((e.target as HTMLAnchorElement).style.color = "#aaaaaa")
                  }
                >
                  info@ijebuigbodiaspora.org
                </a>
              </li>
              <li>
                <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "#555555" }}>
                  Donations & Support
                </div>
                <a
                  href="mailto:support@ijebuigbodiaspora.org"
                  className="text-sm transition-colors duration-200"
                  style={{ color: "#aaaaaa" }}
                  onMouseEnter={(e) =>
                    ((e.target as HTMLAnchorElement).style.color = "#C9A227")
                  }
                  onMouseLeave={(e) =>
                    ((e.target as HTMLAnchorElement).style.color = "#aaaaaa")
                  }
                >
                  support@ijebuigbodiaspora.org
                </a>
              </li>
              <li>
                <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "#555555" }}>
                  Headquarters
                </div>
                <span className="text-sm" style={{ color: "#aaaaaa" }}>
                  Ijebu Igbo, Ogun State, Nigeria
                </span>
              </li>
            </ul>
          </motion.div>
        </div>

        {/* Bottom bar */}
        <div
          className="mt-16 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4"
          style={{ borderTop: "1px solid #222222" }}
        >
          <p className="text-sm" style={{ color: "#555555" }}>
            © {new Date().getFullYear()} Ijebu Igbo Descendants in Diaspora. All rights reserved.
          </p>
          <p className="text-xs" style={{ color: "#444444" }}>
            Built with ❤️ for Ijebu Igbo
          </p>
        </div>
      </div>
    </footer>
  );
}
